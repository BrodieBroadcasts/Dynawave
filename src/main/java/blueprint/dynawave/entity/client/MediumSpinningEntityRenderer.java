...
public class MediumSpinningEntityRenderer extends EntityRenderer<SpinningBarEntity> {
    private final MediumBarModel model;

    public MediumSpinningEntityRenderer(EntityRendererFactory.Context ctx) {
        super(ctx);
        // Load the custom model from a model loader
        this.model = new MediumBarModel(ctx.getPart(ModModelLayers.MEDIUM_SPINNING_BAR_LAYER));
    }

    @Override
    public Identifier getTexture(SpinningBarEntity entity) {
        return new Identifier(MOD_ID, "textures/entity/medium_spinning_bar.png");
    }
