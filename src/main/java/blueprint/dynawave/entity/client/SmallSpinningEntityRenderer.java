...
public class SmallSpinningEntityRenderer extends EntityRenderer<SpinningBarEntity> {
    private final SmallBarModel model;

    public SmallSpinningEntityRenderer(EntityRendererFactory.Context ctx) {
        super(ctx);
        // Load the custom model from a model loader
        this.model = new SmallBarModel(ctx.getPart(ModModelLayers.SMALL_SPINNING_BAR_LAYER));
    }

    @Override
    public Identifier getTexture(SpinningBarEntity entity) {
        return new Identifier(MOD_ID, "textures/entity/small_spinning_bar.png");
    }
